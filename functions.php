...
if (!defined('AXIOFIT_THEME_DIR_URI')) {
    define('AXIOFIT_THEME_DIR_URI', trailingslashit( esc_url( get_stylesheet_directory_uri() )));
}
...
require_once AXIOFIT_THEME_DIR . 'vendor/autoload.php';
...
/*---------------------- Theme Styles ---------------------------*/
/*---------------------------------------------------------------*/
function axiofit_enqueue_styles() {
	wp_enqueue_style( 'axiofit', get_stylesheet_directory_uri() . '/style.css', array() );
}

add_action( 'wp_enqueue_scripts', 'axiofit_enqueue_styles' ); 
